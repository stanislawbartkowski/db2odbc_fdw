...
static void db2_GetForeignPaths(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid)
{
	...
	logdebug("After create path %p",path);

    logdebug("baserel 2 %p",baserel->pathlist);

    ListCell *p1;
    foreach(p1, baserel->pathlist) {

    }
    logdebug("before add path %p",p1);

//    baserel->pathlist =
//             list_insert_nth(baserel->pathlist, 0, path);
//    logdebug("after insert");


	add_path(baserel, path);
	logdebug("After addpath");
